...
public class ExHibition2Admin {
    public static void compositeFile4OssParam2Excel(List<TaskExcel> excelExportList, LocalDateTime currentDateTime, DateTimeFormatter dateTimeFormatter) {
        ...
        //给首行赋值
        for (int k = 0; k < OutportExcel.ADMIN_EXCEL_HEADER.length; k++) {
            HSSFCell headerCell = headerRow.createCell(k);
            headerCell.setCellValue(OutportExcel.ADMIN_EXCEL_HEADER[k]);
            headerCell.setCellStyle(headerStyle);
            if(OutportExcel.ADMIN_EXCEL_HEADER[k].equals("主图") || OutportExcel.ADMIN_EXCEL_HEADER[k].equals("店铺名称（非掌柜名）")){
        ...
        for(int i=0;i < excelExportList.size(); i++){
            ...
            if(excelExportList.get(i).getMyPicture() != null){
                ...
                MyPicture myPicture = excelExportList.get(i).getMyPicture();
